...
describe("live ops DB-backed feature provider", () => {
  ...
  it("fails closed when orderbooks are stale even if trades are fresh", async () => {
    const events = createFeatureFixtureEvents().filter((event) =>
      event.type !== "ORDERBOOK" || event.exchangeTimestamp !== "2026-05-25T00:21:00.000Z"
    );
    const reader = new FakeFeatureWindowReader(events);
    ...
    expect(snapshot.metadata).toMatchObject({
      latestEventAt: "2026-05-25T00:21:00.000Z",
      latestEventLagMs: 0,
      latestOrderbookEventAt: "2026-05-25T00:20:00.000Z",
