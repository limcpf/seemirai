...
const requiredUnsafeSourceScanPatterns = [
  { label: "ord_type price market-buy payload", pattern: /[\x27"]?ord_type[\x27"]?\s*[:=]\s*[\x27"]?price/u },
  ...
  { label: "snake_case price/best order artifact", pattern: /[\x27"]?order_type[\x27"]?\s*[:=]\s*[\x27"]?(PRICE|price|BEST|best)/u },
  { label: "camelCase price/best order artifact", pattern: /[\x27"]?orderType[\x27"]?\s*[:=]\s*[\x27"]?(PRICE|price|BEST|best)/u },
  { label: "withdrawal enabled true", pattern: /[\x27"]?withdrawal_enabled[\x27"]?\s*[:=]\s*true/u },
  { label: "deposit enabled true", pattern: /[\x27"]?deposit_enabled[\x27"]?\s*[:=]\s*true/u },
  { label: "Upbit deposit API path", pattern: /\/v1\/deposits/u },
  { label: "Upbit withdrawal API path", pattern: /\/v1\/withdraws/u },
  { label: "futures enabled true", pattern: /[\x27"]?futures_enabled[\x27"]?\s*[:=]\s*true/u },
  { label: "leverage enabled true", pattern: /[\x27"]?leverage_enabled[\x27"]?\s*[:=]\s*true/u },
  { label: "market order enabled true", pattern: /[\x27"]?market_order_enabled[\x27"]?\s*[:=]\s*true/u },
  { label: "entry market order enabled true", pattern: /[\x27"]?entry_market_order_enabled[\x27"]?\s*[:=]\s*true/u },
];
const requiredSecretSourceScanPatterns = [
  { label: "database url literal", pattern: /SEEMIRAI_DATABASE_URL\s*=\s*postgres:\/\/[^\s<:]+:[^\s<@]+@/u },
  { label: "postgres credential url literal", pattern: /postgres(?:ql)?:\/\/[^:<\s"\x27]+:[^@<\s"\x27]+@/u },
  { label: "upbit access key literal", pattern: /SEEMIRAI_UPBIT_ACCESS_KEY\s*=\s*[^<\s]+/u },
  { label: "upbit secret key literal", pattern: /SEEMIRAI_UPBIT_SECRET_KEY\s*=\s*[^<\s]+/u },
  { label: "upbit accessKey property literal", pattern: /[\x27"]?accessKey[\x27"]?\s*:\s*[\x27"][A-Za-z0-9._-]{16,}[\x27"]/u },
  { label: "upbit secretKey property literal", pattern: /[\x27"]?secretKey[\x27"]?\s*:\s*[\x27"][A-Za-z0-9._\/=+-]{16,}[\x27"]/u },
  { label: "upbit access_key property literal", pattern: /[\x27"]?access_key[\x27"]?\s*:\s*[\x27"][A-Za-z0-9._-]{16,}[\x27"]/u },
  { label: "upbit secret_key property literal", pattern: /[\x27"]?secret_key[\x27"]?\s*:\s*[\x27"][A-Za-z0-9._\/=+-]{16,}[\x27"]/u },
  { label: "telegram bot token literal", pattern: /SEEMIRAI_TELEGRAM_BOT_TOKEN\s*=\s*[0-9]+:[A-Za-z0-9_-]+/u },
  { label: "legacy telegram bot token literal", pattern: /\bTELEGRAM_BOT_TOKEN\s*=\s*[0-9]+:[A-Za-z0-9_-]+/u },
  { label: "TUI control token literal", pattern: /SEEMIRAI_TUI_CONTROL_TOKEN\s*=\s*[^<\s]+/u },
  { label: "authorization bearer literal", pattern: /[\x27"]?Authorization[\x27"]?\s*[:=]\s*[\x27"]?(Bearer|bearer)\s+[A-Za-z0-9._-]+/u },
  { label: "lowercase authorization bearer literal", pattern: /[\x27"]?authorization[\x27"]?\s*[:=]\s*[\x27"]?(Bearer|bearer)\s+[A-Za-z0-9._-]+/u },
  { label: "raw compact JWT literal", pattern: /\beyJ[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+\b/u },
  { label: "JWT field compact literal", pattern: /[\x27"]?jwt[\x27"]?\s*[:=]\s*[\x27"]eyJ[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+[\x27"]/u },
