...
market/strategy에서 여러 live trade event가 5분 안에 발생해도 서로를 cooldown으로 숨기지 않는다. restart/crash/recovery는
...
넣고, 수동 점검 event는 reason/evidence별로 서로 다른 P1 cooldown key를 갖는다. broker 제출 불확실성이나 reservation release
실패처럼 주문별 수동 reconcile이 필요한 event는 reservation/attempt evidence까지 cooldown key에 넣는다. 이 dispatch는 주문 판단 결과를 바꾸지 않는
