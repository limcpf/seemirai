...
알 수 없는 scope가 포함되면 잔고/미체결 주문 조회도 호출하지 않고 live execution의 broker guard 차단으로 수렴한다.
...
cleanup artifact에 cancel 요청 시각, redacted broker order suffix, terminal 조회 실패 사유를 남겨 closeout/manual review가 거래소 side
effect를 추적할 수 있게 한다. post-cleanup reconcile/PnL/status summary의 `budgetUsedKrw`는 preflight snapshot 값만 쓰지 않고, 현재
attempt의 durable reservation notional이 반영된 값을 하한으로 사용한다.
