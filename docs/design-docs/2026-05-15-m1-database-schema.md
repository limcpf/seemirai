...
| `live_reconcile_exchange_order_snapshots` | PostgreSQL | reconcile 시점의 거래소 주문 상태 snapshot. 같은 run/exchange_order_id는 중복 insert되지 않는다. identifier가 확인된 주문은 `exchange_order_id` 관측 전후가 달라도 run/identifier unique index로 중복 insert를 차단한다. |
