...
| `live_reconcile_exchange_order_snapshots` | PostgreSQL | reconcile 시점의 거래소 주문 상태 snapshot. 같은 run/exchange_order_id는 중복 insert되지 않는다. identifier가 확인된 주문은 `exchange_order_id` 관측 전후가 달라도 run/identifier unique index로 중복 insert를 차단한다. 두 식별자가 모두 있는 bridge snapshot은 같은 run의 uuid-only/identifier-only partial snapshot을 하나의 row로 정규화한다. 빈 문자열 식별자는 identity로 허용하지 않는다. |
...
| `live_reconcile_position_snapshots` | PostgreSQL | exchange/market/strategy 단위 복구 후보와 평균단가 산출 근거를 append-only로 저장한다. 평균단가 근거가 없으면 `MANUAL_REVIEW_REQUIRED` snapshot만 남기고 `positions` 갱신을 허용하지 않는다. 양수 수량을 `RECOVERABLE`로 저장하려면 평균단가도 양수여야 한다. 같은 run/exchange/market/strategy/captured_at/source 조합은 unique index로 중복 insert를 차단한다. |
