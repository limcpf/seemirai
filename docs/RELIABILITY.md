...
- why summary는 사용자-facing 한국어 상태/원인/영향/필요 조치를 먼저 배치하고, 내부 식별자는 `trace`에 분리한다. DB 조회 실패 시 endpoint 전체를 실패시키지 않고 해당 하위 객체의 `readStatus`를 `UNAVAILABLE`로 낮춘다.
- LLM summary는 deterministic evidence를 대체하지 않는다. LLM provider timeout, invalid JSON, output size 초과, provider 장애는 `EXPLANATION_FAILURE` evidence로만 저장하며, 주문 판단을 변경하지 않는다.
