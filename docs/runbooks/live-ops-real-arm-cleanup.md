...
| PnL/status | `readStatus=OK`, 명시적인 완료 계열 snapshot status, provider read 완료 후 시각 기준 30초 freshness를 모두 만족해야 손실 증거로 쓰고, 결측/오래됨/PARTIAL/status 누락/read-level `OK` status는 0으로 보정하지 않는다. preflight 직후 새 row의 1초 이내 future skew만 허용한다 |
