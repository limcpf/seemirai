...
| 제품 요구사항 | B+ | PRD, 기능 요구사항, MVP 업무 명세가 Upbit KRW paper trading 범위와 비범위를 고정하며, M8-C 및 M9 #68 closeout 증거와 #102 calibration 후보 산출물이 정합된다. | 비활성 proposal을 운영 기본값으로 활성화하려면 별도 calibration approval PR에서 동일 run shape 전후 비교를 붙인다. |
| 설계 기록 | B | 런타임, DB, TypeScript 모듈 구조, 운영 가드레일 결정이 설계 문서와 실행 계획에 남아 있다. | M9 이후 runtime 조립 경계가 바뀌면 design-docs에 추가한다. |
| 실행 계획 운영 | B+ | M0~M8 MVP 계획의 완료 상태와 Post-M8 계획이 정합되며, M9 paper 운영 베타 closeout과 #102 비활성 calibration proposal이 active plan에 반영됐다. | phase 1.5/v0.2 전환 전 threshold activation 승인 기준과 운영 비교 증거를 분리한다. |
