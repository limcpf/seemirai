...
- 2026-05-21: issue #42 Sub PR 3은 `issue-42/03-alerts-telegram-cooldown`에서 Telegram `sendMessage` outbound adapter, plain text formatter, 4096자 전송 제한, provider timeout/failure reason, notifier exception 정규화, alert fingerprint 구분자 escape, P0/P1 durable cooldown과 atomic delivery reservation, 실패 시 reservation 즉시 해제, P2/P3 memory cooldown, severity escalation bypass, kill switch control accepted 전이의 alert runtime wiring, P0/P1 retry job 후보와 notification failure threshold reason code를 고정한다. Telegram inbound command, daily report aggregator, retry worker 실행, 24시간 soak script는 후속 PR 범위로 유지한다.
