...
| 2 | `issue-22/02-risk-persistence` | `order_events` migration/schema/repository, state transition event mapper, append persistence integration test | risk limit evaluator, `risk_ok` runtime 연결, PaperBroker 실행 | `migrations/**`, `src/infrastructure/db/**`, persistence tests, DB schema docs | `corepack pnpm exec vitest run tests/integration`, `./scripts/verify` |
