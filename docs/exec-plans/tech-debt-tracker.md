...
| ID | 상태 | 영향 | 근거 | 권장 처리 |
| --- | --- | --- | --- | --- |
| TD-001 | calibration deferred | 전략 후보 생성 설명력은 보강됐지만 운영 기본 threshold calibration은 #68 관측 결과에 묶여 남아 있음 | M11 Sub PR 1-4에서 candle momentum, realized volatility, volume spike, depth slope, depth 변화율, VWAP 이탈, 체결 방향 누적 imbalance, market regime, 시간대별 유동성 filter, cost-adjusted expected return feature의 contract, calculator, backtest/paper parity, strategy integration을 추가했다. 2026-05-26 기준 #68 72시간 관측 artifact가 없어 운영 기본 threshold 보정값은 확정하지 않는다. | #68 완료 후 `docs/design-docs/2026-05-26-m11-calibration-closure.md`의 비교 기준으로 별도 calibration issue 또는 PR에서 threshold 후보를 검토한다. |
