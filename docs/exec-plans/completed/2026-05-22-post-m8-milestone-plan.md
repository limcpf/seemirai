...
M16 상태: completed. issue #143의 완료 기록은 [`2026-06-02-issue-143-m16-live-reconcile.md`](./2026-06-02-issue-143-m16-live-reconcile.md)를 기준으로 한다. Sub PR 01-08이 mother branch로 merge됐고, 08 closeout에서 전체 검증(typecheck, 903 tests, verify docs/hooks/github) 통과, DB integration 48 tests 통과, Upbit read-only REST/private WebSocket live reconcile smoke 통과, live order API 0회 확인을 완료했다. Final PR #152 review drain 중 runtime review fix 이후 disposable DB 기반 DB integration 48 tests와 Upbit live REST/WebSocket smoke 4 tests를 재실행했고, PR #152는 main에 merge됐다. closeout 보강으로 `finish-readiness-audit`는 PR 생성 전 readiness 감사 기준에 맞게 정리했고, PR 미생성 자체는 PASS/FAIL/PARTIAL 판정에서 제외한다. read-only reconcile runtime, private WebSocket myOrder/myAsset, REST snapshot bootstrap, mismatch fail-closed를 포함한다. M17 PnL 회계, M18 판단 이유 ledger, M19 exit engine, M20 Telegram inbound, M21 수동 승인, M22 autonomous trading은 M16 범위 밖이다.
