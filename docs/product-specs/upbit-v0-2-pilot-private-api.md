...
| 개별 주문 조회 | `주문조회` | 허용 | 같은 smoke run의 `identifier`로 생성/취소 결과 확인 |
| 주문 생성 | `주문하기` | 조건부 허용 | `PILOT_ORDER_SMOKE`와 별도 order guard, 고유 `identifier` 필요 |
| 주문 취소 | `주문하기` | 조건부 허용 | 같은 smoke run의 `identifier`만 |
