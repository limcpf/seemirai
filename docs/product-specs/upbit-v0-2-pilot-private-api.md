...
| `PILOT_READ_ONLY` | 계정 잔고와 주문 조회 contract 확인 | `GET /v1/accounts`, `GET /v1/order` | 없음 |
...
`PILOT_ORDER_SMOKE`는 `PILOT_READ_ONLY`와 `PILOT_POLICY_SYNC`가 실패하면 시작하지 않는다. 주문 생성 후 취소가 실패하거나 주문 상태를 확인하지 못하면 추가 주문을 시도하지 않고 manual review 상태로 수렴한다.
...
5. 체결 가능성을 낮춘 보수적 지정가 주문을 1회 생성한다.
