...
const liveOpsObservedFactSchema = {
  type: "object",
  required: ["statusLabel", "message", "observedAt", "action", "trace"],
  properties: {
    statusLabel: { type: "string" },
    message: { type: "string" },
...
const liveOpsStatusSummarySchema = {
  type: "object",
  required: [
    "status",
    "statusLabel",
  ...
  properties: {
    ...
    impact: { type: ["string", "null"] },
    action: { type: ["string", "null"] },
    mode: { enum: ["dry_run", "heartbeat_only", "live_armed", "live_order_capable"] },
    liveEnabled: { type: "boolean" },
    liveOrderCapable: { type: "boolean" },
    ...
    latestHeartbeat: liveOpsObservedFactSchema,
    latestCandidate: liveOpsObservedFactSchema,
    latestDecision: liveOpsObservedFactSchema,
    latestOrderAttempt: liveOpsObservedFactSchema,
    latestFillOrCancel: liveOpsObservedFactSchema,
    reconcile: {
      type: "object",
      required: ["result", "mismatchCount", "openOrderCount", "lastReconcileAt", "actionRequired"],
      properties: {
        result: { type: "string" },
        mismatchCount: { type: ["number", "null"] },
        openOrderCount: { type: ["number", "null"] },
    ...
    budget: {
      ...
      properties: {
        ...
        dailyAutonomousNotionalLimitKrw: { type: "string" },
        maxOpenPositionNotionalKrw: { type: "string" },
        dailyNotionalUsedKrw: { type: ["string", "null"] },
        openExposureKrw: { type: ["string", "null"] },
        realizedPnlKrw: { type: ["string", "null"] },
        unrealizedPnlKrw: { type: ["string", "null"] },
      },
      additionalProperties: false,
    },
    riskBlock: {
      type: "object",
      required: ["killSwitchState", "newOrdersBlocked", "requiresManualReview", "blockedReason"],
      properties: {
    ...
    alertRetry: {
      type: "object",
      required: ["statusLabel", "lastSentAt", "lastSkippedAt", "action"],
      properties: {
        statusLabel: { type: "string" },
    ...
    response: {
      200: {
        ...
        properties: {
          ...
          liveOps: liveOpsStatusSummarySchema,
          why: {
            anyOf: [
              { type: "null" },
              {
